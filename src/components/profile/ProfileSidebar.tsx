      ...
      return (
        <button
          ...
          className={cn(
            ...
            isActive 
              ? "bg-gradient-to-r from-coral-100 to-peach-100 text-coral-700 shadow-md border border-coral-200" 
        ...
        >
          ...
          <Icon className={cn(
            ...
            isActive ? "text-coral-600" : "text-muted-foreground group-hover:text-foreground"
          )} />
          <span className={cn(
            "font-medium transition-colors text-sm",
            isActive ? "text-coral-700" : "group-hover:text-foreground"
          )}>
            {tab.label}
          </span>
        </button>
...
export const ProfileSidebar = ({ activeTab, onTabChange, className }: ProfileSidebarProps) => {
  return (
    <>
      {/* Desktop Sidebar */}
      <div className={cn(
        "hidden lg:block w-72 bg-gradient-to-br from-white to-mint-50/30 rounded-2xl shadow-lg border border-white/20 h-fit sticky top-8",
        className
      )}>
        ...
        <Sheet>
          <SheetTrigger asChild>
            <Button 
              variant="outline" 
              size="sm" 
              className="mb-6 bg-white/80 hover:bg-white border-white/40 shadow-sm"
            >
